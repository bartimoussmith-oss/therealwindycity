"""Miller Tapes theater — shared by the front door and the nav page.

`render_theater()` plays every Charles Miller intervention back-to-back with
Up-Next cards on loop, plus the sidebar sorting booth (kind / type / incident
flags / meeting / text search / order) and a read-only queue with per-clip
tape links. Used in two places:

  * the app front door (Ledger HQ's first chapter — see streamlit_app.py and
    its twin Transparency_Index_App.py), called as render_theater(embed=True)
  * pages/2_Miller_Tapes.py, the standalone nav page (embed=False + masthead)

Sound: browsers forbid unmuted autoplay with zero user interaction, so the
picture starts muted (guaranteed motion on load) and the player unmutes on
the visitor's FIRST gesture anywhere on the page (click / tap / keypress) —
no dedicated button hunt. It also retries unmuting a few seconds in, which
covers return visitors whose browser has granted the site autoplay-with-sound
(Media Engagement Index). Stdlib + streamlit only.
"""
...
font-size:15px;padding:8px 16px;cursor:pointer">\U0001F50A SOUND ON</button>
<span style="font-size:13px">Picture's rolling &mdash; click/tap ANYWHERE once and the audio starts.</span>
</div>
</div>
...
function sound(){if(player&&player.unMute){try{player.unMute();player.setVolume(100);}
catch(e){}$("mt-sound").style.display="none";}}
function early(){if(player&&player.unMute){try{player.unMute();player.setVolume(100);}
catch(e){}}}
["pointerdown","keydown","touchend"].forEach(function(ev){
window.addEventListener(ev,function(){sound();},{capture:true});});
...
started=true;
setTimeout(early,1500);setTimeout(early,4000);
...
def render_theater(embed=False):
    """Full Miller Tapes theater: booth + auto-playing player + queue."""
    try:
        items = _load()
...
<p>484 TURNS &middot; 132 INTERRUPTIONS &middot; 20 TAPES &middot; 616 CUED BLOCKS</p></div>""",
        ...
        st.subheader("\U0001F3AC The Miller Tapes — every intervention, playing now")
        st.caption("Picture starts instantly; click/tap **anywhere once** for sound. "
                   "Sorting booth lives below the chapters in the sidebar.")

    type_order = [t for t in ("testimony", "statement", "formal_warning",
                              "interruption", "response")
                  if any(r["type"] == t for r in items)]
    meetings = sorted({f"{r['date']} \u00b7 {r['title']}" for r in items})
    kinds_all = sorted({r["mkind"] for r in items})

    # --------------------------------------------------------- sorting booth
    st.sidebar.header("\U0001F3AA Sorting booth")
    collapse = st.sidebar.toggle("Collapse replay angles", value=True, key="mt_collapse",
                                 help="On: 286 unique plays. Off: all 616 blocks, "
                                      "including double-angle replays of the same moment.")
    kinds = st.sidebar.multiselect("Meeting kind", kinds_all, default=kinds_all,
                                   key="mt_kinds")
    ...
    incidents = st.sidebar.toggle("Incidents only", value=False, key="mt_incidents",
                                  help="Only mic cuts, points of order, time calls, "
                                       "and interruption blocks.")
    meet = st.sidebar.multiselect("Meeting", meetings, default=meetings, key="mt_meet")
    query = st.sidebar.text_input("Search the words", value="", key="mt_query",
                                  help="Case-insensitive substring match on the "
                                       "transcript text.")
    sort = st.sidebar.radio("Order", ["Chronological", "Longest first", "Shuffle"],
                            horizontal=True, key="mt_sort")
    if sort == "Shuffle":
        if "mt_seed" not in st.session_state:
            st.session_state.mt_seed = 0
        if st.sidebar.button("\U0001F500 Reshuffle", key="mt_reshuffle"):
