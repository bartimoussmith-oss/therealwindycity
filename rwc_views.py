"""rwc_views — shared view library for The Real Windy City multipage app.

Every feature is a function here; pages/*.py are thin wrappers and
streamlit_app.py is the home page that previews each feature live.

Original single-file docstring follows.
...
<b>Screening Room</b> page (sidebar, under the console).</p></div>""",
        ...
        st.page_link("pages/16b_Screening_Room.py", label="🎬 Open Screening Room")
        st.markdown(
...
def render_ledger_header():
    """Masthead + live metrics + breaking banner. Returns stats for the home grid."""
    _comic_boot()
    _comic_masthead()
    ...
    return eng_stats, ver_rows, found_n, last


def render_section(name):
    """Render one SECTIONS entry (single view or tabbed group)."""
